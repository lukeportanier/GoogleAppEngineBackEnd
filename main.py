...
def GetResults():
    ...
    with conn.cursor() as cursor:
        cursor.execute(f"SELECT number, instance_id FROM storagedata")
    ...
    # Get the instances that generated the smallest and largest numbers
    smallest_instance = [row[1] for row in results if row[0] == smallest_number][0]
    largest_instance = [row[1] for row in results if row[0] == largest_number][0]

    # Close the database connection
    conn.close()

    return f"Smallest Number: {smallest_number} Was Generated by Instance {smallest_instance}. \n\nLargest Number: {largest_number} Was Generated by Instance {largest_instance}: "
